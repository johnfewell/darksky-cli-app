...
attr_accessor :name, :later, :later_desc, :now_one_word, :input, :url, :days_array, :week_summary
  ...
  #gets the page from the web and stores it in an instance variable
  def get_page(url)
  ...
  def days_temps
    ...
    @week_summary = @days_summary.shift

    @days_array << @days_names
    @days_array << @days_temps_max
    @days_array << @days_temps_min
    @days_array << @days_summary
  end
